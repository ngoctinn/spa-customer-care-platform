...
def create_new_role(*, session: Session = Depends(get_db_session), role_in: RoleCreate):
    """Tạo một vai trò mới (yêu cầu quyền 'manage_roles')."""
    return roles_service.create_role(db_session=session, role_in=role_in)


@router.get("/roles", response_model=List[RolePublicWithPermissions])
def get_all_roles_with_permissions(session: Session = Depends(get_db_session)):
    """Lấy danh sách tất cả các vai trò và quyền của chúng."""
    return roles_service.get_all_roles(db_session=session)
...
def delete_a_role(
    *,
    ...
    """Xóa một vai trò."""
    roles_service.delete_role(db_session=session, db_role=role)
    return
...
def assign_permission(
    *,
    ...
    return roles_service.assign_permission_to_role(
        db_session=session, role_id=role_id, permission_id=assignment.permission_id
    )
...
def remove_permission(
    *,
    ...
    return roles_service.remove_permission_from_role(
        db_session=session, role_id=role_id, permission_id=permission_id
    )


@router.post("/permissions", response_model=PermissionPublic)
def create_new_permission(
    *, session: Session = Depends(get_db_session), permission_in: PermissionCreate
):
    """Tạo một quyền mới."""
    return roles_service.create_permission(
        db_session=session, permission_in=permission_in
    )


@router.get("/permissions", response_model=List[PermissionPublic])
def get_all_permissions(session: Session = Depends(get_db_session)):
    """Lấy danh sách tất cả các quyền."""
    return roles_service.get_all_permissions(db_session=session)
...
def assign_role_to_a_user(
    *,
    ...
    return users_service.assign_role_to_user(
        db_session=session, user_id=user_id, role_id=assignment.role_id
    )
...
def remove_role_from_a_user(
    *,
    ...
    return users_service.remove_role_from_user(
        db_session=session, user_id=user_id, role_id=role_id
    )
