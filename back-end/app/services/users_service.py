...
def create_online_user(db: Session, *, user_in: UserCreate) -> User:
    """
...
def update_user(db: Session, *, db_user: User, user_in: UserUpdateMe) -> User:
    """
...
def assign_role_to_user(
    ...
    role = roles_service.get_role_by_id(db, role_id=role_id)
...
def create_staff_account(
    db: Session, *, user_in: AdminCreateStaffRequest
) -> User:
    """
    ...
    role = None
    if user_in.role_id:
        role = roles_service.get_role_by_id(db, role_id=user_in.role_id)
    else:
        role = roles_service.get_role_by_name(
            db, name=DefaultRoles.STAFF
    ...
    try:
        ...
        if not role:
            ...
            role = roles_service.create_role(db, role_in=role_to_create)
...
def update_user_by_admin(
    db: Session, *, db_user: User, user_in: UserUpdateByAdmin
) -> User:
    """
...
def delete_user_by_id(db: Session, *, user_to_delete: User) -> User:
