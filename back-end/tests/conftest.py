...
def staff_profile(db_session: Session, staff_user: User) -> StaffProfile:
    # SỬA LỖI: Cung cấp `full_name` khi tạo StaffProfile
    profile_data = StaffProfileCreate(
        user_id=staff_user.id,
        full_name="Staff User Full Name",
        phone_number="0987654321",
...
def another_user(db_session: Session) -> User:
    """Fixture tạo một người dùng thứ hai trong DB."""
    user_data = User(
...
@pytest.fixture(scope="function")
def another_user_profile(db_session: Session, another_user: User) -> Customer:
    """Fixture tạo một hồ sơ khách hàng đã được liên kết với 'another_user'."""
    customer_profile = Customer(
        full_name="Another User Profile",
