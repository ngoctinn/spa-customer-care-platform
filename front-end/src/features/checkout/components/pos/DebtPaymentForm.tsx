...
import apiClient from "@/lib/apiClient"; // Giả sử apiClient đã có
...
// API function (cần được tạo trong file api tương ứng)
const payDebt = async (
  payload: DebtPaymentFormValues & { customerId: string }
) => {
  const { customerId, ...data } = payload;
  return apiClient(`/customers/${customerId}/pay-debt`, {
    method: "POST",
...
export function DebtPaymentForm() {
  ...
  const payDebtMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["customers"] }); // Invalidate list
      setCustomer(null); // Reset
      form.reset();
    },
    onError: (error: Error) => {
